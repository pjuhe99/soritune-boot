...
try {
    ...
    // created_at 을 명시적으로 핀: 1초 sleep 회피 + 결정론적 정렬
    $db->prepare("UPDATE notices SET created_at = ? WHERE id = ?")->execute(['2099-01-01 10:00:00', $ida]);
    $db->prepare("UPDATE notices SET created_at = ? WHERE id = ?")->execute(['2099-01-01 10:00:01', $idb]);
    $db->prepare("UPDATE notices SET created_at = ? WHERE id = ?")->execute(['2099-01-01 10:00:02', $idc]);

    $rows = noticeListAdmin($db, $cohortIdA);
    t('listAdmin 3건', count($rows) === 3);
    t('listAdmin 0번: 최신 노출', (int)$rows[0]['id'] === $idc);
    t('listAdmin 1번: 오래된 노출', (int)$rows[1]['id'] === $ida);
    t('listAdmin 2번: 숨김', (int)$rows[2]['id'] === $idb);

    $rowsM = noticeListMember($db, $cohortIdA);
    t('listMember 2건 (숨김 제외)', count($rowsM) === 2);
    t('listMember 0번: 최신', (int)$rowsM[0]['id'] === $idc);
} finally {
