...
- **행 부재 = 무등급(0)·추가횟수 0** 과 동치. backfill 은 등급 보유자만 행 생성, 그 외엔 필요 시(잠금·부여) lazy 생성.
- member_key 규약은 slice6 `bravoAttemptMemberKey()` 재사용 (user_id 우선, 없으면 `p:`+phone).
- **extra_attempts 를 이 테이블에 두는 이유 (리뷰 P1-3):** `bravo_member_settings` 는 `user_id NOT NULL` 키라 phone-only 회원이 추가 횟수를 받을 수 없는 구멍이 생김. 사람 단위 진실원인 이 테이블(member_key 키)에 두면 등급·횟수·잠금이 한 행으로 해결. (`bravo_member_settings` 는 무수정 — override/granted 의 user_id 한계는 기존 그대로, 범위 외.)
- **이 행은 등급당 quota 의 동시성 mutex 로도 사용** (§4-2 참조).
...
| `bravoGradeLockRow(PDO, string $memberKey): array` | **quota mutex**: `INSERT IGNORE`(level 0 기본행) 후 `SELECT ... FOR UPDATE` — 트랜잭션 안에서만 호출, 사람 단위 직렬화의 안정적 잠금 대상 (행이 항상 존재하므로 빈 범위 gap-lock 한계 없음) |
...
`bravoHasReleasedPass` 는 **삭제** (slice8 도입 — 이 게이트로 대체. 테스트도 함께 정리).
...
- **분기 순서 변경 (같은 시험 재응시로 생기는 공존 상태):** 이제 released 결과(과거 attempt)와 **새 in_progress attempt 가 공존** 가능 (강등/불합격 후 같은 시험 재시작). 카드 분기는 ① `in_progress`(이어하기/제출 마무리) → ② `result`(합격/불합격 표시 — 불합격이고 시험 open + quota 잔여 + 미보유면 `다시 도전하기` 버튼 동반) → ③ `held` → ④ 도전하기 순으로 재정렬 — slice8 의 "result 최상단" 원칙을 "진행 중 응시 우선" 으로 대체 (진행 중인 시험을 가리면 안 됨).

## 7. 관리자 UI (admin-bravo.js 자격 탭 확장 + admin.php)

- `bravo_member_list` 응답에 `member_key` + `current_level` + `extra_attempts_{1,2,3}` + 등급별 누적 used 추가.
- 자격 테이블에 컬럼 추가: **현재 등급**(select 0~3 — 수동 조정, 로그 source=`admin_adjust`, ref=admin_id), **추가횟수 B1/B2/B3**(number 입력).
- **저장 경로 분리 (리뷰 P1-3):** 등급 조정·추가횟수는 신규 case `bravo_grade_update`(member_key 기준 — 서버가 응답에 내려준 member_key 를 그대로 받음, `bravo_member_grades` upsert)로 저장 — **phone-only 회원도 동작**. 기존 `bravo_member_update`(user_id 기준, override/granted/notes)는 무수정 — phone-only 의 override/granted 한계는 기존 그대로(범위 외).
...
- `tests/bravo_member_grades_schema_invariants.php`: 신규 2테이블 (grades 의 extra_attempts 3컬럼 포함).
- `tests/bravo_grade_canonical_test.php`:
  - bravoGradeSet/CurrentLevel/로그, no-op(같은 레벨), demote 단계·0 가드, bravoGradeLockRow lazy 생성
  - ApplyExamPass: released 훅(bravoExamUpdate 경유) — pass 만 승급·fail 제외·기존 상위등급 no-op·재전환 멱등
  - 게이트: 보유 차단(+강등 후 해제), 이전등급 요건(B2 에 B1 필요, B1 은 면제, grandfather 보유 인정), 누적 quota(**서로 다른 같은-등급 시험 2개 합산** 3회 소진→차단→extra_attempts 부여→통과), **같은 시험 재응시**(released 후 새 attempt_no 허용 / 미발표 submitted 는 차단), 동시 시작 race($testHook 으로 mutex 검증 — 같은-등급 다른 시험 2개 시나리오), **phone-only 회원**(p: 키로 등급·extra·강등 전부)
...
- **회귀 = 기존 19파일 + 신규 2 = 21파일** — ⚠️ slice8 의 `bravo_release_test.php` 는 차단 의미 교체로 **단언 수정 필요** (released-pass 차단 → 보유 차단, 불합격자 같은-시험 재응시 단언 추가; 발표 전 비공개 회귀 단언은 유지). slice6 `bravo_attempt_test.php` 의 submitted 잠금·attempt_limit 단언도 새 의미로 수정.
- 브라우저 검증(사용자): 강등 모달 풀사이클, 누적 횟수 표시, 관리자 등급 조정·추가횟수, 홈 카드 등급 반영.

## 10. 파일 구조

- **Create** `migrate_bravo_member_grades.php` (테이블 2 + backfill — `bravo_member_settings` 무수정)
...
dev 에만. 운영 일괄 반영 시 **마이그 선실행 — pull 보다 먼저** (리뷰 P2-4):

```
boot-prod 에서:
1. git fetch origin
2. git checkout origin/main -- migrate_bravo_member_grades.php   ← 마이그 파일만 선반입
3. php migrate_bravo_member_grades.php                            ← 신규 테이블+backfill (구코드와 무충돌 — 구코드는 이 테이블을 모름)
4. git pull origin main                                           ← 새 코드 활성화 (테이블이 이미 있어 무중단)
```

마이그가 추가형(신규 테이블 CREATE + backfill)뿐이라 선실행이 구코드에 무해. 반대 순서(pull 먼저)는 새 표시 경로가 미존재 테이블을 조회하는 구간이 생기므로 금지. backfill 은 그 시점 PROD `bravo_grade` 값을 읽음 — pull 전이라 freeze 도 아직 미적용이지만 backfill 의 `GREATEST` upsert 멱등이라 재실행으로 보정 가능.
