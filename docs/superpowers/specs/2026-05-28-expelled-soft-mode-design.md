...
- 단 체크리스트·현황판 탭 (leader/coach/operation/head 모두 노출되는 탭) 에서만 기본 숨김. 상단의 "내보내기 회원 포함" 체크박스로 노출 토글
- 과제율 계산도 `active` 와 동일하게 반영
- **신규 expel 케이스** 부터 의도대로 동작. 기존 PROD expelled 는 `group_id=NULL` 인 상태로 두므로 토글 효과 제한적 (§7.4 / §9.2 / §12.1 참조)

운영자가 부담 없이 "내보내기" 를 활용할 수 있는 가벼운 시각적 분리 도구가 된다. 동시에 PROD 의 과거 expelled 회원들이 cron·코인·외부연동 경로로 다시 통과하기 시작하므로 — 단순 UI 변경이 아닌 **데이터 재처리 정책 변경** 이라는 성격을 가진다 (§12 참조).

## 2. 비목표

- `expelled` enum 값 자체 제거 — **하지 않음** (의미 유지, 동작만 약화)
- `/operation` **회원 관리 탭** (`#tab-members`) 의 UI/필터 변경 — **하지 않음** (expelled 별도 탭/카운트 그대로). 단, operation 도 가지고 있는 **체크리스트·현황판 탭** 에는 신규 토글이 함께 노출됨 (체크리스트·현황판은 leader/coach/operation/head 모든 role 에 공통 노출되며 동일 동작이 의도).
- `out_of_group_management` 동작 변경 — **하지 않음**
- `refunded` 동작 변경 — **하지 않음**
- 신규 컬럼/플래그 추가 — **하지 않음** (DB 마이그 0개)
- 기존 PROD expelled 회원의 `group_id` 자동 복구 — **하지 않음** (수동 배정 — §9.2 참조)
...
체크리스트·현황판 UI 는 `public_html/js/bootcamp.js` 가 소유한다 (`loadChecklist` / `loadStatusBoard` / `loadChecklistByMission` + `renderChecklist*` / `renderStatusBoard`). `/leader` 와 `/admin` 두 페이지 모두 이 파일을 로드하고, 체크리스트·현황판 탭은 admin.js 의 role 분기 4곳 (operation, coach, leader/subleader, head 등) 어디에나 노출되므로 한 곳만 수정하면 **모든 role 에서 동일하게 토글 표시·동작** 한다 (의도). `/operation` 의 회원 관리 탭 (`#tab-members`) 은 별도 코드라 영향 없음.
...
### 7.4 한계 — 기존 expelled (group_id=NULL) 회원

이 토글은 SQL WHERE 절에서 `bm.member_status != 'expelled'` 조건만 빼는 방식이다. **`group_id` 필터에는 손대지 않는다.**

따라서:

- **신규 expel 케이스** (이 spec 배포 후 expel 된 회원): expel 시 `group_id` 가 보존됨 → leader 가 자기 조로 필터하든, 운영자가 전체 조회하든 토글 ON 으로 정상 노출.
- **기존 expel 케이스** (이 spec 배포 전 expel 돼서 이미 `group_id=NULL` 인 회원): leader 가 자기 조 (`?group_id=N`) 로 필터하면 토글 ON 이어도 **여전히 안 보임** (group_id NULL 매칭 안 됨). 운영자가 group 필터를 안 걸고 전체 조회하면 보임.

운영자가 "기존 내보낸 회원도 leader 화면 토글에 보이게" 하려면, `/operation` 의 회원 관리 탭에서 해당 회원의 `group_id` 를 다시 배정해야 함. 이는 의도된 운영 흐름 (Q2 결정: 자동 복구 안 함).

스펙이 약속하는 토글의 효과는 **"신규 expelled 가 group 기반 필터로 사라지지 않는다"** 까지. 기존 expelled 의 "조 미배정으로 인한 자동 제외" 는 별개 이슈로 운영자가 명시적으로 복구해야 한다.
...
마이그 안 함. /operation 에서 운영자가 필요 시 개별 조 배정.

**토글에 대한 효과 (§7.4 와 연결)**: 기존 expelled 는 `group_id=NULL` 상태라, leader 가 자기 조 필터를 적용하면 토글을 켜도 보이지 않음. 즉 본 spec 의 토글은 신규 expel 케이스에 대해서만 완전 작동. 기존 expelled 를 leader 토글로 보이게 하려면 운영자가 먼저 조 배정 필요.

**소급 적용되는 영향** (§12.1 표 참조 — 자세한 영향과 PROD 대응은 §12.2 사전 배포 체크리스트):
- 코인 적립 (coin_reward_group): 자동 적립 cron 이 INACTIVE 목록에서 빠지므로 적립 시작. 단 group 기반 일부 로직은 group_id 가 없으면 0 적립 가능 → 실제 파급 적음.
- 카페·줌 cron 백필: 다음 cron 실행 시 미반영분 백필.
- 후기 등록: 가능해짐.
- 멤버페이지: 노출 시작.
...
### 12.1 기존 expelled 회원에 대한 소급 적용 (중요)

이 변경은 단순 UI 토글이 아니라 **과거 expelled 회원에 대한 데이터 재처리 정책 변경** 이다. 배포 직후부터 PROD 의 모든 기존 expelled 회원은 `active` 와 동일한 시스템 경로를 다시 통과한다. 사용자 Q4 결정 ("그게 변경의 의도") 에 따라 이를 의도된 동작으로 진행하되, 다음 파급 효과를 명시적으로 인지하고 배포해야 한다:

| 경로 | 즉시 영향 | 잠재 파급량 |
|------|---------|-----------|
| `coin_reward_group` 정산·적립 cron | 다음 cron 실행부터 expelled 가 INACTIVE 목록에서 빠짐 → 진행 중 cycle 의 코인 산정에 포함 | `group_id=NULL` 이면 group 기반 적립은 0이지만, group 기반이 아닌 cycle 보너스가 있다면 소급 가산 가능. 적립 규칙 별 검토 필요 |
| `cron.php initDailyChecks` / `backfillChecks` | 다음 cron 부터 expelled 도 일일 체크 row 생성·백필 대상 | 출석·과제 미수행이 누적된 회원에 대해 점수 감점이 새로 발생할 수 있음 |
| `attendance.php` 출석 집계 | 즉시 분모 포함 | 출석률 통계가 즉시 변동 |
| `member_page.php` (부티즈) | 즉시 노출 | UI 상 회원 수 증가 |
| `review.php` 후기 등록 | 즉시 가능 | (회원 자발적 동작이라 자동 폭증 없음) |
| `qr_actions.php`, `qr.php` | 즉시 입장 가능 | (회원 자발적 동작) |
| `cafe_ingest.php resolveMemberByKey` | 다음 카페 cron 부터 해당 회원의 카페 글 ingest | **누락된 과거 글이 한 번에 백필될 수 있음** (잠재 대량 처리) |
| `integration.php` (zoom 등) | 다음 cron 부터 백필 | 카페와 동일. zoom 미반영 분 한꺼번에 처리 가능 |

### 12.2 사전 배포 체크리스트 (PROD 배포 전 필수)

DEV 푸시 직후·PROD 머지 전에 운영자 (또는 Claude) 가 다음을 확인하고 결정한다:

1. **PROD expelled 회원 수·분포 확인**
   ```sql
   SELECT
     COUNT(*) AS total,
   ...
   - `total > 0 AND with_group = 0`: 대부분 group 의존 처리는 0 → 영향 작음. 그러나 카페 백필·후기·멤버페이지 등 group 무관 경로는 즉시 영향. 운영자가 인지 후 배포
   - `total > 0 AND with_group > 0`: 보기 드문 케이스. 코인 적립·점수 계산·체크리스트 모두 즉시 영향. 회원 명단 출력 후 운영자 판단

2. **카페·줌 cron 백필 윈도우 확인**
   - `cron.php` 의 백필 함수가 얼마나 과거까지 거슬러 올라가는지 확인 (`backfillChecks` 의 윈도우 파라미터)
   - 위 SQL 의 `total > 0` 이면 cron 첫 실행 직후 INSERT 량 추적 (`integration_logs` 등)

3. **opt-out 옵션 (필요 시)**
   기존 expelled 회원이 많고 소급 처리를 차단해야 할 경우, 배포 직전 다음 중 하나를 적용:
   - **(a) status 임시 분리**: 기존 expelled → 임시 enum 값 (e.g., `expelled_legacy`) 으로 마이그 후 새 코드는 `expelled` 만 active 처럼 처리. 다음 spec 으로 분리.
   - **(b) 운영자가 사전에 기존 expelled 를 `refunded` 등으로 정리**: 가장 단순. 사용자 결정 필요.
   - **(c) 코인·cron 백필만 skip 하는 별도 가드 추가**: 코드 복잡도 ↑. 비추.

   본 spec 의 기본은 **opt-out 없이 진행** (Q4 결정).

### 12.3 PROD 즉시 영향 (체크리스트 통과 후)

- 체크리스트·현황판: 기본 토글 off 로 expelled 노출 안 됨 (UI 변화는 즉시 드러나지 않음).
- 운영자가 "내보내기" 버튼 confirm 메시지에서 새 동작 인지 가능.
- 신규 expel 케이스부터 `group_id` 보존 → 향후 토글이 의도대로 동작.
...
단, 배포 후 cron 이 한 번 이상 돈 뒤 롤백하면 그동안 INSERT 된 체크/적립 row 는 그대로 남는다. 데이터 클린업 SQL 은 별도 작성 필요 (롤백 시점에 영향받은 회원 명단 + INSERT 시각 기준으로 DELETE).
