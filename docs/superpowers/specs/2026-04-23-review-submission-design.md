...
- 타입당 **active(= `cancelled_at IS NULL`) 제출 1건 제한**. 이미 active 제출이 있는 타입은 API에서 거부.
- 취소된 건은 유니크 제약 대상에서 제외되므로 DB상 재제출이 기술적으로 가능하나, **회원 UI에서는 재제출 경로를 열지 않음** (운영자가 취소했다는 건 해당 건이 거짓이라는 판단이므로).

### 3.2 대상 회원 scope

- **제출 가능 조건**: `bootcamp_members.is_active = 1` AND `member_status NOT IN ('refunded','leaving','out_of_group_management')` AND **현재 active cycle의 cohort가 회원의 cohort와 일치 또는 걸쳐있음** (즉, active cycle에 `member_cycle_coins` row를 만들 수 있는 회원).
- 11기 단독 참여 회원(= 12기 미참여)에게는 바로가기 카드 자체를 숨김. 제출해도 코인이 12기 cycle에 쌓여 실질적으로 소실될 위험이 있기 때문.
- 판정은 서버에서: active cycle의 `cohort_id`와 회원의 `cohort_id` 매칭. 매칭되면 eligible.

### 3.3 취소 정책

- **운영자만** 취소 가능. 회원은 본인 제출을 취소/수정할 수 없음.
- 취소는 **소프트 삭제** — `cancelled_at`, `cancelled_by`, `cancel_reason` 채우고 row는 유지. 감사 목적.
- 취소 시 `applyCoinChange` 음수 호출로 같은 cycle에서 5코인 차감. `coin_logs`에 `coin_change = -5`, `reason_type = "review_cafe"` 또는 `"review_blog"`, `reason_detail = "cancel:review_submission_id:{id} reason:{...}"` 기록.
- 기존 `coinReasonLabel()` 로직이 음수 변동에 " (취소)" 접미를 자동 부착하므로 회원 코인 내역에도 "카페 후기 (취소)" / "-5"로 노출됨.

### 3.4 cycle cap 상호작용

- `applyCoinChange`는 해당 cycle `max_coin`(기본 200) 초과분을 자동 클램프. `applied < 5` 인 경우 응답에 실제 `applied` 값을 그대로 반영하고, 회원 Toast에 "(이번 기수 최대치 도달)" 안내.
- 실무상 캡 도달 케이스는 드묾.
...
- **유니크 제약**(동일 회원 + 동일 타입 + active 1건): MySQL 조건부 유니크 미지원. **애플리케이션 레벨에서 트랜잭션 내 선조회**(`SELECT ... WHERE member_id=? AND type=? AND cancelled_at IS NULL FOR UPDATE`) 후 INSERT로 해결. 이중 클릭/동시 제출 방어.
- `coin_amount`는 정책 변경 대비 row에 박제. 현재는 항상 5.
...
- `submitted`: 해당 타입의 `cancelled_at IS NULL` row. 없으면 `null`.
...
- 검증 순서:
  ...
  3. eligible (cohort 일치 등). 아니면 에러.
  4. `url` 포맷: 정규식 `^https?://` + 길이 10~500. 아니면 에러 `"URL 형식이 올바르지 않습니다"`.
  5. 트랜잭션 시작 → `SELECT ... FOR UPDATE`로 active 제출 없음 확인. 있으면 에러 `"이미 제출하셨습니다"`.
- 처리(트랜잭션):
  1. `getActiveCycle()`로 cycle 조회. 없으면 에러.
  2. `INSERT INTO review_submissions (member_id, cycle_id, type, url, coin_amount) VALUES (?, ?, ?, ?, 5)`.
  3. `applyCoinChange($db, $memberId, $cycleId, 5, "review_{type}", "review_submission_id:{insertId}", null)`. `adminId`는 null(회원 self-action).
  4. commit.
- 응답: `{success, applied_coin: result.applied, submission: {id, url, submitted_at, coin_amount}, message: "..." }`.
- `applied < 5`이면 메시지에 `"이번 기수 최대치에 도달하여 {applied}코인만 적립되었습니다"` 포함.
...
- 처리(트랜잭션):
  ...
  2. `applyCoinChange($db, $member_id, $cycle_id, -$coin_amount, "review_{type}", "cancel:review_submission_id:{id} reason:{cancel_reason}", $admin_id)`.
  3. commit.
- 응답: `{success, applied_coin: -5, message: "후기가 취소되고 코인 5이 회수되었습니다"}`.
...
4. 회원 1명·운영자 1명으로 엔드투엔드 회귀 테스트:
   - 카페/블로그 각 제출 → 코인 +5 적립 확인 → 코인 내역 화면에 "카페 후기 +5" 표시.
   - 운영 탭에서 목록 조회 → 취소 → 코인 -5 확인 → 내역에 "카페 후기 (취소) -5".
   - 토글 off 시 화면 반영 확인.
...
3. **Cycle cap(200) 도달**: 후기 +5가 캡에 걸려 `applied < 5`인 경우 응답에 실제 applied 반영 및 Toast 안내. 실무상 드묾.
4. **취소 시 earned 음수 가능**: 해당 cycle earned가 5 미만이면 음수. 회계상 정합하나 회원 내코인 화면에 `-3` 같은 earned 표시가 뜰 수 있음. 현재 `MemberCoinHistory`는 earned 그대로 렌더. 일단 허용(스펙 외 클램프).
...
9. **eligible 판정의 `cohort_id` 가정**: "active cycle의 `cohort_id` == `bootcamp_members.cohort_id`"로 대상 회원을 구분한다. 이는 `bootcamp_members` 한 row가 **현재 참여 중인 단일 cohort만** 가리킨다는 가정에 기반. 11기→12기 전환 회원의 `cohort_id`가 실제로 12기로 업데이트되는지, 또는 회원이 여러 cohort에 걸쳐 별도 row를 갖는지 **구현 단계에서 DB 스키마와 운영 데이터로 확인**. 가정이 틀리면 eligible 판정 로직을 "회원의 참여 cohort 집합에 active cycle의 cohort가 포함" 방식으로 보강해야 함. 회귀 테스트에 11기 단독/12기 단독/양쪽 걸친 케이스 모두 포함.
